...
class DataSet(Dataset):
    def __init__(self, args, transform=None):
        ...
        digital_img_dir = os.path.join(data_dir, dataset)
        ...
        self.style = f'{digital_img_dir}/style'
        self.smooth =  f'{digital_img_dir}/smooth'
        ...
        print(f'Dataset: real {len(self.image_files[self.photo])} style {self.len_digital_img}, smooth {self.len_smooth}')

    def __len__(self):
        return self.debug_samples or len(self.image_files[self.photo])

    @property
    def len_digital_img(self):
    ...
    def __getitem__(self, index):
        ...
        anm_idx = index
        if anm_idx > self.len_digital_img - 1:
            anm_idx -= self.len_digital_img * (index // self.len_digital_img)

        digital_img, digital_img_gray = self.load_digital_img(anm_idx)
        smooth_gray = self.load_digital_img_smooth(anm_idx)

        return image, digital_img, digital_img_gray, smooth_gray
    ...
    def load_digital_img(self, index):
    ...
    def load_digital_img_smooth(self, index):
...
if __name__ == '__main__':
    ...
    digital_img_loader = DataLoader(DataSet('dataset/Hayao/smooth'), batch_size=2, shuffle=True)

    img, img_gray = iter(digital_img_loader).next()
